...
colrbutn proc hParent:DWORD,Instance:DWORD,lpText:DWORD,
  ...
  ; -------------------------------------------
    ...
    invoke 	wsprintf,ADDR bcCnt,ADDR szdToSF,cCounter
  ...
  ; -------------------------------------------
    ...
    invoke 	lstrcat,ADDR bClass,ADDR cName
    invoke 	lstrcat,ADDR bClass,ADDR bcCnt
...
TxtColor proc hParentWnd:DWORD,hpInstance:DWORD,lpText:DWORD,tx:DWORD,ty:DWORD,wd:DWORD,ht:DWORD,txtClr:DWORD,ID:DWORD
	
	LOCAL 	wcx:WNDCLASSEX
	LOCAL 	hBrush:DWORD
	LOCAL 	bClass[32]:BYTE
	LOCAL 	bCnt[8]:BYTE
	LOCAL 	hTxtColorWnd:DWORD
	
	invoke 	CreateSolidBrush,txtClr
	mov 	hBrush,eax
	
	.data
		txtColorClass 	db "anTxtColor",0
		txtColCounter	dd 0
	.code
	
	inc 	txtColCounter
	mov 	bClass[0],0
	invoke 	wsprintf,ADDR bCnt,ADDR szdToSF,txtColCounter
	invoke 	lstrcat,ADDR bClass,ADDR txtColorClass
	invoke 	lstrcat,ADDR bClass,ADDR bCnt
	
	mov 	wcx.cbSize,	sizeof 	WNDCLASSEX
	mov 	wcx.style, CS_BYTEALIGNCLIENT or CS_BYTEALIGNWINDOW
	m2m 	wcx.lpfnWndProc,offset TxtColorProc
	mov 	wcx.cbClsExtra,0
	mov 	wcx.cbWndExtra,40
	m2m 	wcx.hInstance,hpInstance
	m2m 	wcx.hbrBackground, COLOR_BTNFACE + 1
	...
	invoke 	CreateWindowEx,WS_EX_LEFT,ADDR bClass,lpText,WS_VISIBLE OR WS_CHILD,tx,ty,wd,ht,hParentWnd,0,hpInstance,0
	mov 	hTxtColorWnd,eax
	
	invoke 	SetWindowLong,hTxtColorWnd,4,txtClr
	
	invoke 	ShowWindow,hTxtColorWnd,SW_SHOW
	invoke 	UpdateWindow,hTxtColorWnd
	
	mov 	eax,hTxtColorWnd
	ret

TxtColor endp

TxtColorProc proc 	hWin:DWORD,uMsg:DWORD,wParam:DWORD,lParam:DWORD

	LOCAL 	Rct:RECT
	LOCAL 	hBrush,hDC:DWORD
	LOCAL 	ps:PAINTSTRUCT
	LOCAL 	buffer[128]:BYTE
	...
	.elseif 	eax == WM_PAINT
		...
		invoke 	DrawText,hDC,ADDR buffer,-1,ADDR Rct,DT_RIGHT
